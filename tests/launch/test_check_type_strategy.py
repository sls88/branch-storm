...
def return_1_2_str3() -> List[Union[int, str]]: return [1, 2, "3"]
...
def test_check_type_strategy_nested_on_operation():
    with pytest.raises(
            TypeError,
            match=re.escape(
...
def test_check_type_strategy_nested_on_branch():
    with pytest.raises(
            TypeError,
            match=re.escape(
                "Operation: enriched_job -> br1 -> receive_and_pass_all.\n"
        ...
        br("enriched_job")[
            ...
            br("br1")[
                op(obj(receive_and_pass_all)(m[List[int]])),
            ].check_type_strategy_all(True)
        ].check_type_strategy_all(False).run()


def test_check_type_strategy_nested_on_branch_receive():
    with pytest.raises(
            TypeError,
            match=re.escape(
                "Operation: enriched_job -> receive_and_pass_all.\n"
        ...
        br("enriched_job")[
            ...
            obj(receive_and_pass_all)(m[List[int]])
        ].check_type_strategy_all(True).run()
